...
def grid_show(to_shows, cols,index="0"):
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            axs[i, j].set_xticks([])
    fig.tight_layout()
    plt.savefig("png/{}.png".format(index))
...
def visualize_heads(att_map, cols,index):
    to_shows = []
    ...
    grid_show(to_shows, cols=cols,index=index)

def visualize_Layer(att_map, cols):
  for i in  range(len(att_map)):
    for j in  range(att_map[i].shape[0]):
      visualize_heads(att_map[i][j],cols,"{}_{}".format(j+1,i+1))
    #   break
    # break
def add_channel(input):
  plug=torch.zeros((128,126,1))
  return torch.cat((input,plug),dim=2)


def saveCompare(raw,test,index):
    raw=add_channel(raw)
    fig, axs = plt.subplots(3, 3,figsize=(10,10))
    axs[0, 0].imshow(raw[:,:,0], aspect='auto',cmap='gray')
    axs[1, 0].imshow(raw[:,:,1], aspect='auto',cmap='gray')
    axs[2, 0].imshow(raw, aspect='auto',cmap='gray')

    test=add_channel(test)
    axs[0, 1].imshow(test[:,:,0], aspect='auto',cmap='gray')
    axs[1, 1].imshow(test[:,:,1], aspect='auto',cmap='gray')
    axs[2, 1].imshow(test, aspect='auto',cmap='gray')

    axs[0, 2].imshow(raw[:,:,0]-test[:,:,0], aspect='auto',cmap='gray')
    axs[1, 2].imshow(raw[:,:,1]-test[:,:,1], aspect='auto',cmap='gray')

    fig.tight_layout()
    # plt.show()
    plt.savefig("png/{}.png".format(index))

def saveBatch(raw,test):
    raw =raw.detach()
    test =test.detach()
    for i in range(len(raw)):
        saveCompare(raw[i],test[i],"{}_inout".format(i+1))
